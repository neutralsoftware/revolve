...
void Broadway::executeTWI(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (trap) {
        Logger::log("Broadway", LogLevel::Error,
                    "TWI instruction triggered a trap. CIA: " +
...
void Broadway::executeORI(uint32_t field0, uint32_t field1, uint32_t,
                          int32_t simm) {
    uint32_t ra = field1;
...
void Broadway::executeORIS(uint32_t field0, uint32_t field1, uint32_t,
                           int32_t simm) {
    uint32_t ra = field1;
...
void Broadway::executeXORI(uint32_t field0, uint32_t field1, uint32_t,
                           int32_t simm) {
    uint32_t ra = field1;
...
void Broadway::executeXORIS(uint32_t field0, uint32_t field1, uint32_t,
                            int32_t simm) {
    uint32_t ra = field1;
...
void Broadway::executeANDI_(uint32_t field0, uint32_t field1, uint32_t,
                            int32_t simm) {
    uint32_t ra = field1;

    uint32_t a = (ra == 0) ? 0 : state.gpr[ra];
    uint32_t result = a & static_cast<uint32_t>(simm);
    state.gpr[field0] = result;

    state.updateCR0(result);
}

void Broadway::executeANDIS_(uint32_t field0, uint32_t field1, uint32_t,
                             int32_t simm) {
    uint32_t ra = field1;

    uint32_t a = (ra == 0) ? 0 : state.gpr[ra];
    uint32_t result = a & (static_cast<uint32_t>(simm) << 16);
    state.gpr[field0] = result;
...
void Broadway::executeLWZU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "LWZU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeSTWU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "STWU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeSTBU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "STBU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeLHZU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "LHZU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeLHAU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "LHAU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeSTHU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "STHU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
...
void Broadway::executeLFS(uint32_t field0, uint32_t field1, uint32_t,
    ...
    uint32_t value = Bus::read32(effectiveAddress);
    state.fpr[field0] = *reinterpret_cast<float *>(&value);
}
...
void Broadway::executeLFSU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "LFSU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
    ...
    uint32_t value = Bus::read32(effectiveAddress);
    state.fpr[rd] = *reinterpret_cast<float *>(&value);
    state.gpr[ra] = effectiveAddress;
}
...
void Broadway::executeLFD(uint32_t field0, uint32_t field1, uint32_t,
    ...
    state.fpr[field0] = *reinterpret_cast<double *>(&value);
}
...
void Broadway::executeLFDU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "LFDU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
    ...
    uint64_t value = Bus::read64(effectiveAddress);
    state.fpr[rd] = *reinterpret_cast<double *>(&value);
    state.gpr[ra] = effectiveAddress;
}
...
void Broadway::executeSTFS(uint32_t field0, uint32_t field1, uint32_t,
    ...
    uint32_t value = *reinterpret_cast<uint32_t *>(&state.fpr[field0]);
...
void Broadway::executeSTFSU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "STFSU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
    ...
    uint32_t value = *reinterpret_cast<uint32_t *>(&state.fpr[rd]);
    Bus::write32(effectiveAddress, value);
...
void Broadway::executeSTFD(uint32_t field0, uint32_t field1, uint32_t,
    ...
    uint64_t value = *reinterpret_cast<uint64_t *>(&state.fpr[field0]);
...
void Broadway::executeSTFDU(uint32_t field0, uint32_t field1, uint32_t,
    ...
    if (ra == 0 || ra == rd) {
        Logger::log(
            "Broadway", LogLevel::Error,
            "STFDU instruction with RA == 0 or RA == RD is invalid. CIA: " +
                utils::toHexString(state.cia));
        return;
    }
    ...
    uint64_t value = *reinterpret_cast<uint64_t *>(&state.fpr[rd]);
    Bus::write64(effectiveAddress, value);
